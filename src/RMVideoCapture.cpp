 ...
 * File name:
 *
 * Description: RMVideoCapture.cpp
 *
 *
 * Created by:  Author1
...
RMVideoCapture& RMVideoCapture::operator>>(Mat& image)
{
...
void RMVideoCapture::cvtRaw2Mat(const void* data, Mat& image)
{
 if (format == V4L2_PIX_FMT_MJPEG)
 {
   Mat src(capture_height, capture_width, CV_8UC3, (void*) data);
   image = cv::imdecode(src, 1);
 }
 else if (format == V4L2_PIX_FMT_YUYV)
 {
   Mat yuyv(capture_height, capture_width, CV_8UC2, (void*) data);
